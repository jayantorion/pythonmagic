...
async def tailor_resume_for_job(
    ...
    if not master_resume:
        # Fallback to any resume or build default AST
        res_any = await db.execute(select(Resume).where(Resume.profile_id == profile.id).order_by(Resume.created_at.desc()))
    ...
    # 4. Run Tailoring Engine + Diff Guard
    ...
    # 7. Update Application CRM to READY_TO_APPLY
...
async def get_resume_version(version_id: str, db: AsyncSession = Depends(get_db)):
...
async def preview_resume_version_html(version_id: str, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(ResumeVersion).where(ResumeVersion.id == version_id))
    ver = result.scalars().first()
    if not ver:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Resume version not found")
...
async def generate_cover_letter(job_id: str, db: AsyncSession = Depends(get_db)):
    profile = await get_or_create_default_profile(db)
    ...
    # Fetch latest tailored resume AST or master AST
    ver_res = await db.execute(
        select(ResumeVersion).where(ResumeVersion.job_id == job_id).order_by(ResumeVersion.created_at.desc())
    )
    ...
    # Save to application CRM record if present
    app_res = await db.execute(select(Application).where(Application.job_id == job.id))
    app_record = app_res.scalars().first()
    if app_record:
